...
pub(crate) struct Session {
    ...
    pub(super) safe_command_rules: crate::config_types::CommandSafetyRuleset,
...
impl Session {
    ...
    pub fn turn_input_with_history(&self, extra: Vec<ResponseItem>) -> Vec<ResponseItem> {
        ...
        let sanitize_encrypted_reasoning =
            !current_auth_mode.is_some_and(code_app_server_protocol::AuthMode::is_chatgpt);
                    ...
                    match &emission {
                        ...
                        EnvironmentContextEmission::Delta { sequence, delta, snapshot } => {
                            if state.context_timeline.baseline().is_none()
                                && let Err(err) = state.context_timeline.add_baseline_once(snapshot.clone()) {
