...
pub fn start_auto_coordinator(
    ...
    let handle = match builder.spawn(move || {
        ...
        Ok(handle) => handle,
        Err(err) => {
            tracing::error!("auto coordinator spawn failed: {err:#}");
...
fn run_auto_loop(inputs: AutoLoopInputs) -> Result<()> {
    ...
    let mut schema = build_schema(&active_agent_names, schema_features, &allowed_cli_routing_entries);
    ...
    loop {
        ...
        if let Some(conv) = next_conversation {
            ...
            match request_coordinator_decision(DecisionRequest {
                ...
                require_cli_model_routing: schema_features.include_cli_model_routing,
                allowed_cli_routing_entries: &allowed_cli_routing_entries,
            }) {
...
struct DecisionRequest<'a> {
    ...
    model_slug: String,
    require_cli_model_routing: bool,
    allowed_cli_routing_entries: &'a [AutoDriveCliRoutingEntry],
}

#[tracing::instrument(skip_all)]
fn request_coordinator_decision(request: DecisionRequest<'_>) -> Result<ParsedCoordinatorDecision, DecisionFailure> {
    let parse_options = DecisionParseOptions {
        require_cli_model_routing: request.require_cli_model_routing,
        allowed_cli_routing_entries: request.allowed_cli_routing_entries.to_vec(),
    };
...
fn request_decision_with_model(
    ...
    let selected_model = Arc::new(Mutex::new(model_slug.clone()));
    let model_slug_fallback = model_slug.clone();
    let selected_model_for_retry = Arc::clone(&selected_model);
    let mut did_usage_limit_model_fallback = false;
    let classify = move |error: &anyhow::Error| {
            ...
            && client.auto_switch_accounts_on_rate_limit()
                ...
                && let Some(auth_manager) = client.get_auth_manager() {
                    ...
                    let current_account_id = auth
                        .as_ref()
                        .and_then(code_core::CodexAuth::get_account_id)
                        .or_else(|| {
...
fn maybe_compact(request: CompactionRequest<'_>) -> CompactionResult {
    ...
    .is_some();
    if !applied
        && let Some(retry_bounds) = compute_slice_bounds(conversation)
